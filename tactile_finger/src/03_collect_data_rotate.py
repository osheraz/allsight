...
def main():
    ...
    up_to = 8
    max_press_time = 7.0
    max_rotation_time = 7.0
    max_pressure = 2.0
    max_pressure_rotate = 15.0

    save_every_sec = 0.005
    save_every_sec_rotate = 0.2
        ...
        for q in Q:
            ...
            for i in range(start_from, up_to, 1):
                ...
                try:
                    ...
                    # Start the rotate
                    a = np.radians(rot_a)
                    yaw_ = [-a, a, a, -a, 0, 0, 0, 0]
                    pitch_ = [0, 0, 0, 0, -a, a, a, -a]

                    tp1 = env.arm.get_ee_pose()
                    env.arm.move_manipulator.scale_vel(scale_vel=0.02, scale_acc=0.01)
                    ...
                    for k in range(len(yaw_)):
                        ...
                        tp = env.arm.rotate_pose_by_rpy(tp1, 0.0, pitch_[k], yaw_[k], wait=False)
