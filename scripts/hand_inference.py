    ...
    def hand_inference(self):
        ...
        while True:
            ...
            with torch.no_grad():
                ...
                y_right = unnormalize(
                    y_right[0], self.statistics["mean"], self.statistics["std"]
                )

                y_left = self.model(left_raw, left_ref).cpu().detach().numpy()
                y_left = unnormalize(
                    y_left[0], self.statistics["mean"], self.statistics["std"]
                )

                y_bottom = self.model(bottom_raw, bottom_ref).cpu().detach().numpy()
                y_bottom = unnormalize(
                    y_bottom[0], self.statistics["mean"], self.statistics["std"]
                )

            px_right, py_right, pr_right = (
                int(y_right[6]),
                int(y_right[7]),
                max(0, int(y_right[8])),
            )
            px_left, py_left, pr_left = (
                int(y_left[6]),
                int(y_left[7]),
                max(0, int(y_left[8])),
            )
            px_bottom, py_bottom, pr_bottom = (
                int(y_bottom[6]),
                int(y_bottom[7]),
                max(0, int(y_bottom[8])),
            )

            right_w_circ = cv2.circle(
                right_raw, (px_right, py_right), pr_right, (0, 0, 0), 2
            )
            right_w_circ = cv2.putText(
                right_w_circ,
                f"Contact: {[px_right, py_right, pr_right]}",
                (10, 30),
            ...
            left_w_circ = cv2.circle(
                left_raw, (px_left, py_left), pr_left, (0, 0, 0), 2
            )
            left_w_circ = cv2.putText(
                left_w_circ,
                f"Contact: {[px_left, py_left, pr_left]}",
                (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.8,
                (255, 0, 0),
            ...
            bottom_w_circ = cv2.circle(
                bottom_raw, (px_bottom, py_bottom), pr_bottom, (0, 0, 0), 2
            )
            bottom_w_circ = cv2.putText(
                bottom_w_circ,
                f"Contact: {[px_bottom, py_bottom, pr_bottom]}",
                (10, 30),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.8,
                (255, 0, 0),
            ...
            cv2.imshow(
                "Hand View",
                np.concatenate((left_w_circ, right_w_circ, bottom_w_circ), axis=1),
            )
